...
function parseBlocks(text: string): Block[] {
  ...
  while (i < lines.length) {
    ...
    if (paraText) blocks.push({ text: paraText, type: "paragraph" });
  }
 ...
 */
function buildWindows(blocks: Block[]): Array<{ startIdx: number; blocks: Block[] }> {
  const windows: Array<{ startIdx: number; blocks: Block[] }> = [];
  let i = 0;

  while (i < blocks.length) {
    const windowBlocks: Block[] = [];
    let charCount = 0;
    let j = i;

    while (j < blocks.length) {
      const b = blocks[j];
      if (charCount + b.text.length > WINDOW_CHARS && windowBlocks.length > 0) break;
      windowBlocks.push(b);
 ...
 * 本地索引：0 = 窗口内第一块
 */
async function getSplitPointsForWindow(windowBlocks: Block[]): Promise<number[]> {
  ...
  try {
    const parsed = JSON.parse(jsonMatch[0]) as { split_after?: unknown };
 ...
 */
function assembleChunks(blocks: Block[], splitAfterSet: Set<number>): string[] {
  const chunks: string[] = [];
  let buffer: string[] = [];
...
async function smallTextChunk(rawText: string, docType: string): Promise<string[]> {
...
export async function smartChunkWithDeepSeek(
  ...
  for (let i = 0; i < windows.length; i += CONCURRENCY) {
    ...
    const results = await Promise.all(
      batch.map((w) => getSplitPointsForWindow(w.blocks))
    );
