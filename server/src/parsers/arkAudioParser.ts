 ...
 */
export async function parseAudioWithDoubao(
  audioBuffer: Buffer,
  ...
  const utterances: AudioUtterance[] = (asrResult.utterances ?? []).map((u) => ({
 ...
 */
function buildAudioMarkdown(fullText: string, utterances: AudioUtterance[], durationMs: number): string {
  const lines: string[] = [];
 ...
 * 根据文件扩展名推断音频格式参数
 */
export function getAudioFormat(ext: string): "mp3" | "wav" | "ogg" {
  const map: Record<string, "mp3" | "wav" | "ogg"> = {
    mp3: "mp3",
    wav: "wav",
    ogg: "ogg",
    m4a: "mp3",
