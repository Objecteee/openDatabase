 ...
 */
export async function parseVideoWithArk(videoUrl: string, fps = 1): Promise<VideoParseResult> {
  if (!ARK_API_KEY) {
  ...
  let parsed: VideoParseResult["raw"];
  try {
    parsed = JSON.parse(jsonStr) as VideoParseResult["raw"];
  } catch {
 ...
 * 格式：整体摘要 → 时间戳分段 → 实体汇总
 */
function buildVideoMarkdown(data: VideoParseResult["raw"]): string {
  const lines: string[] = [];
  ...
  if (hasEntities) {
    lines.push("## 关键实体汇总");
    lines.push("");
    if (entities.people?.length) lines.push(`**人物**：${entities.people.join("、")}`);
    if (entities.objects?.length) lines.push(`**物体**：${entities.objects.join("、")}`);
    if (entities.locations?.length) lines.push(`**场景**：${entities.locations.join("、")}`);
    if (entities.text_on_screen?.length) lines.push(`**画面文字**：${entities.text_on_screen.join("、")}`);
    lines.push("");
  }
...
function extractOutputText(resp: Record<string, unknown>): string | null {
    ...
    for (const item of output) {
      if (item.type === "message" && item.content) {
        ...
        if (Array.isArray(content)) {
          const textPart = content.find((c: Record<string, unknown>) => c.type === "output_text") as Record<string, unknown> | undefined;
