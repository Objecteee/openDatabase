...
function parseEnrichItem(item: RawEnrichItem): EnrichmentResult {
...
async function enrichBatch(chunkTexts: string[]): Promise<EnrichmentResult[]> {
  ...
  return chunkTexts.map((_, idx) => {
    ...
    if (!item || typeof item !== "object") return defaultEnrichment();
...
export async function enrichChunks(
  ...
  const batches: Array<{ startIdx: number; items: typeof chunks }> = [];
  ...
  for (let i = 0; i < batches.length; i += BATCH_CONCURRENCY) {
    ...
    const batchResults = await Promise.all(
      concurrentBatches.map((b) => enrichBatch(b.items.map((c) => c.content)))
    );
