...
import { parsePdfWithMistralOcr, type OcrPage } from "../parsers/mistralOcrParser.js";
 ...
 */
function buildPdfPagePointers(
  pages: OcrPage[],
  fullMarkdown: string,
): Array<{ page: number; startOffset: number; endOffset: number }> {
  const result: Array<{ page: number; startOffset: number; endOffset: number }> = [];
  let searchFrom = 0;
 ...
 */
function inferPdfPointer(
  chunkContent: string,
  fullText: string,
  pagePointers: Array<{ page: number; startOffset: number; endOffset: number }>,
): string | null {
  if (pagePointers.length === 0) return null;
  ...
  // fallback：取最近的页
  const closest = pagePointers.reduce((prev, cur) =>
    Math.abs(cur.startOffset - pos) < Math.abs(prev.startOffset - pos) ? cur : prev,
  );
 ...
 */
function extractTimestampPointer(chunkContent: string): string | null {
...
async function parseMarkdownText(
  ...
  documentId: string,
  type: "txt" | "md",
  pdfPagePointers?: Array<{ page: number; startOffset: number; endOffset: number }> | null,
