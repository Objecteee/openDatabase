...
router.post("/auth/refresh", async (req, res) => {
    ...
    if (!token)
        return res.status(401).json({ error: "无 refresh token" });
    try {
        ...
        const fatal = ["无效", "不存在", "已吊销", "已过期", "invalid", "expired"].some((k) => err.message.toLowerCase().includes(k));
        if (fatal) {
...
router.get("/auth/_debug/refresh-cookie", (req, res) => {
    const token = req.cookies?.[cookieName];
    if (!token)
