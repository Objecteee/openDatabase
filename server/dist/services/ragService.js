...
export async function analyzeIntent(messages) {
    const lastUserMsg = [...messages].reverse().find((m) => m.role === "user");
    if (!lastUserMsg)
        return { type: "chat", rewrittenQuery: "", keywords: [] };
 ...
 */
export async function hybridSearch(queryEmbedding, keywords, userId, documentIds) {
    ...
    for (const c of vectorResults) {
        const key = c.id;
        seen.set(key, { ...c, score: c.similarity ?? 0.5 });
    }
    // 关键词命中的 chunk 提升权重
 ...
 */
export async function buildRagContext(messages, queryEmbedding, userId, documentIds) {
