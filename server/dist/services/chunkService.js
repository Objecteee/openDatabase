...
export async function insertMultiVectorChunks(chunks) {
    if (!supabase)
        throw new Error("Supabase 未配置");
    const rows = [];
...
export async function searchChunks(embedding, options) {
    ...
    const payload = fn === "match_chunks_filtered_scoped"
 ...
 */
export async function searchChunksByKeywords(keywords, options) {
    if (!supabase)
        throw new Error("Supabase 未配置");
    if (!options?.userId)
        throw new Error("userId 必填");
